...
| Вопрос  | Ответ |
| ------------- | ------------- |
| Какое значение будет присвоено переменной `c`?  | В данном скрипте переменной c не будет присвоено значение из-за ошибки TypeError.  |
| Как получить для переменной `c` значение 12?  | Для получения значения 12 в переменной c, необходимо привести переменную b к типу int и выполнить операцию сложения, пример на скрине ниже:  |
| Как получить для переменной `c` значение 3?  | Невозможно получить значение 3 для переменной c, так как в данном контексте переменная a имеет значение 1, а переменная b имеет строковое значение "2". Сложение числа и строки будет иметь смысл только в том случае, если строка содержит цифры, например, "1" или "2". , тут вопрос с подвохом и я почти уверен, что большинство студентов ответят  a + int(b)  |
![Снимок](https://user-images.githubusercontent.com/123774335/231794832-af26e318-ed3b-4083-9399-ae16a2b6ed79.PNG)
